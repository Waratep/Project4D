...
class HoughTransform:
    ...
    def _debug(self,debug = False):
        if (debug):            
            cv2.putText(self.frame,str(round(self.slope_right + self.slope_left,3)),(100,25), self.font, 0.5,(255,255,255),2,cv2.LINE_AA)
    ...
    def run(self,image_cap,debugs = False):
        
        self.frame = image_cap
        self.corerun()                    
    ...
    def corerun(self): 
        ...
        self.frame = cv2.resize(self.frame,(100, 70)) 
        ...
        for i in range(width):        
            if maskR[int(height/2)][i] == 255: 
                self.countR+=1
            if self.countR > 10:
                break

        # Hough Transform
        lines = cv2.HoughLinesP(crop,rho = 1,theta = 1*np.pi/180,threshold = 50,minLineLength = 10,maxLineGap = 250)
        ...
        # cv2.putText(frame,str(count),(50,25), font, 0.5,(255,255,255),2,cv2.LINE_AA)
        self.frame=cv2.cvtColor(self.frame, cv2.COLOR_BGR2RGB)    
        cv2.imshow('frame', self.frame)
